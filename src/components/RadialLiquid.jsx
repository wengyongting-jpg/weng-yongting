...
export default function RadialLiquid() {
  ...
  useEffect(() => {
    ...
    const draw = () => { const { width, height } = canvas.getBoundingClientRect(); const cx = width * (.5 + (pointer.x - .5) * .11); const cy = height * (.5 + (pointer.y - .5) * .11); const radius = Math.min(width, height) * .47; ctx.clearRect(0, 0, width, height); ctx.fillStyle = "#050507"; ctx.fillRect(0, 0, width, height); for (let i = 6; i > 0; i--) { const wobble = Math.sin(time * .7 + i * 1.9) * 14; const r = radius * (i / 6) + wobble; const g = ctx.createRadialGradient(cx - r * .22, cy - r * .28, r * .03, cx, cy, r); g.addColorStop(0, `rgba(255,255,255,${.055 + i * .012})`); g.addColorStop(.48, "rgba(35,35,41,.18)"); g.addColorStop(.77, "rgba(235,235,240,.16)"); g.addColorStop(.88, "rgba(255,255,255,.025)"); g.addColorStop(1, "rgba(5,5,7,0)"); ctx.fillStyle = g; ctx.beginPath(); ctx.ellipse(cx + Math.cos(time + i) * 8, cy + Math.sin(time * .8 + i) * 8, r * 1.1, r * .78, time * .1 + i * .12, 0, Math.PI * 2); ctx.fill(); } const shine = ctx.createRadialGradient(cx - radius * .28, cy - radius * .38, 1, cx - radius * .28, cy - radius * .38, radius * .8); shine.addColorStop(0,"rgba(255,255,255,.48)"); shine.addColorStop(.09,"rgba(255,255,255,.1)"); shine.addColorStop(.42,"rgba(255,255,255,0)"); ctx.fillStyle=shine; ctx.fillRect(0,0,width,height); time += .012; frame=requestAnimationFrame(draw); };
    resize(); draw(); window.addEventListener("resize", resize); canvas.addEventListener("pointermove", move); return () => { cancelAnimationFrame(frame); window.removeEventListener("resize", resize); canvas.removeEventListener("pointermove", move); };
  }, []);
  return <canvas ref={canvasRef} className="radial-liquid" />;
}
